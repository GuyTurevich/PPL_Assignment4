...
export async function makeReactiveTableService<T>(sync: (table?: Table<T>) => Promise<Table<T>>, optimistic: boolean): Promise<ReactiveTableService<T>> {
    ...
    let observerArray:Array<Function> = [];
    const UpdateObservers = (Table:Table<T>)=> {
        observerArray.forEach(observerfunction => {
            observerfunction(Table)
        });
    }
    ...
    const handleMutation = async (newTable: Table<T>) => {
        if(optimistic){ 
            ...
            Promise.reject("__EXPECTED_FAILURE__")
            }
            // .then((tableVal)=>{UpdateObservers(tableVal);  Promise.resolve()})
            // .catch(()=>Promise.reject("__EXPECTED_FAILURE__"))
        }
    ...
    return {
        ...
        set(key: string, val: T): Promise<void> {
            let newTable : Record<string, T> =_table;
            let newpair = {} as Record<string,T>
            newpair[key]=val;
            return handleMutation({...newTable,...newpair})
        },
        delete(key: string): Promise<void> {
            let newTable : Record<string, T> =_table; 
            const { [key]: value, ...changedTable } =newTable
            return handleMutation(changedTable)
        },
